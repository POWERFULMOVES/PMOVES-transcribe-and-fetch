...
import LlmModelSelect from '@/components/llm/LlmModelSelect'; // Import the new component
...
export default function FetchContentPage({ initialActiveMainTab = "fetchContent" } = {}) {
  ...
  // Add state for the selected LLM model alias
  const [selectedLlmModelAlias, setSelectedLlmModelAlias] = useState("");

  // Handler for when a model is selected in the LlmModelSelect component
  const handleLlmModelSelect = (alias) => {
    setSelectedLlmModelAlias(alias);
  };
  ...
  const [formState, setFormState] = useState({
    ...
    crawl4aiLlmProviderModel: "", // This will now be managed by the LlmModelSelect component's state
    crawl4aiLlmApiToken: "",
    crawl4aiLlmBaseUrl: "",
  ...
  const handleFetchContent = async () => {
    ...
    // Start building parameters common to both engines
    const params = new URLSearchParams({
      url: formState.url,
      fetch_depth: formState.fetchDepth,
      target_content_area: formState.targetContentArea,
      // target_selector needs to be based on targetContentArea for both engines
      ...
      // Common boolean/basic options, assume they apply or are ignored by backend based on engine
      ...
      // NOTE: extract_tables and extract_images seem common now, include here
      extract_tables: formState.extractTables,
      extract_images: formState.extractImages, // Keep this in formState for UI control
      clean_format: formState.cleanFormat, // Seems common
      json_response: formState.jsonResponse, // Seems common
      // remove_images is derived from extractTextOnly/extractImages, include here
      remove_images: removeImagesBackend,

      // Add fetching engine to params
      engine: fetchingEngine, // Add fetchingEngine to params
    });

    // Add engine-specific parameters conditionally
    if (fetchingEngine === 'jina') { // Use isolated state
      // Jina-specific parameters
      // Note: target_selector_advanced is only relevant when target_content_area is 'advanced'
      if (formState.targetContentArea === 'advanced' && formState.targetSelectorAdvanced) {
         params.append('target_selector_advanced', formState.targetSelectorAdvanced);
      }
      // params.append('json_response', formState.jsonResponse.toString()); // Already in common params
      // params.append('clean_format', formState.cleanFormat.toString()); // Already in common params
      params.append('browser_engine', formState.browserEngine);
      params.append('token_budget', formState.tokenBudget.toString());
      // params.append('remove_images', removeImagesBackend.toString()); // Already in common params
      params.append('cache_ttl', formState.cacheTtl.toString());
      // browser_viewport needs formatting
      ...
      if (formState.crawl4aiViewportWidth !== undefined) params.append('crawl4ai_viewport_width', formState.crawl4aiViewportWidth.toString());
      if (formState.crawl4aiViewportHeight !== undefined) params.append('crawl4ai_viewport_height', formState.crawl4aiViewportHeight.toString());
      ...
      if (formState.crawl4aiPageTimeout !== undefined) params.append('crawl4ai_page_timeout', formState.crawl4aiPageTimeout.toString());
      ...
      // Add crawl4ai specific content extraction parameters
      ...
      // Only include scroll_delay if scan_full_page is true
      ...
      // Add crawl4ai specific debugging and LLM parameters
      ...
      // LLM model is now managed by the state from LlmModelSelect, passed below.
      // No longer need to send crawl4ai_llm_provider_model, crawl4ai_llm_api_token, crawl4ai_llm_base_url
      // via formState.crawl4aiLlm... fields directly.
      ...
      // Add crawl4ai specific expert parameters
      ...
      // Add crawl4ai strategy configurations (must be stringified)
      if (formState.crawl4aiExtractionConfig && formState.crawl4aiExtractionConfig.strategy !== 'none') {
        params.append('extraction_config', JSON.stringify(formState.crawl4aiExtractionConfig));
      }
      if (formState.crawl4aiDeepCrawlConfig && formState.crawl4aiDeepCrawlConfig.strategy !== 'None') {
         // Note: Backend expects 'deep_crawl_strategy', not 'deep_crawl_config' according to usage_with_backend.md
         // Also, the value expected is likely just the strategy name, not the whole config object JSON. Reverting to just sending strategy name.
         // Re-reading usage_with_backend.md, it mentions `deep_crawl_config: JSON.stringify(...)`... Let's stick to the JSON.parse in backend route if it's expecting JSON.
         // Okay, the backend `crawl4ai_fetcher.py` code provided earlier does parse `deep_crawl_config` JSON.
        params.append('deep_crawl_config', JSON.stringify(formState.crawl4aiDeepCrawlConfig));
      }

      // Pass selected LLM model alias from state if crawl4ai is selected
      if (selectedLlmModelAlias) {
          // Note: Backend expects 'llm_model_name'.
          params.append('llm_model_name', selectedLlmModelAlias);
      }

    }

    const url = `${BACKEND_URL}/fetch-content?${params.toString()}`;
    ...
    eventSourceRef.current.onmessage = (event) => {
      ...
      // Update progress and message based on SSE events
      if (data.status) {
        setProgressMessage(data.status);
        setProgressPercent(estimateProgress(data.status));
        // Optionally update other state based on data, e.g., if data includes artifacts
      } else if (data.error) {
        setMainFetchError(data.error.message || "An error occurred during fetch.");
        setProgressMessage(`Error: ${data.error.message || "Unknown error"}`);
        setProgressPercent(0); // Reset on error
        ...
        toast({
          title: "Fetch Error",
          description: data.error.message || "An error occurred while fetching content.",
          variant: "destructive",
        });
      } else if (data.result) {
        // Fetch completed, handle the result
        setFormState(prev => ({ ...prev, result: data.result }));
        setProgressMessage("Fetch Complete");
    ...
    eventSourceRef.current.onerror = (err) => {
      ...
      setMainFetchError("Connection error during fetch. Check backend logs.");
      setProgressMessage("Connection Error");
      setProgressPercent(0);
      setIsFetchingSse(false); // Use renamed state variable
      toast({
        title: "Connection Error",
        description: "Failed to connect to backend fetch service.",
        variant: "destructive",
      });
    };
      ...
      for (const espKey in esp) {
        if (Object.prototype.hasOwnProperty.call(esp, espKey)) {
          ...
          if (espKey === 'browser_viewport') {
            ...
            continue;
          } else if (espKey === 'llm_model_name') { // Handle llm_model_name from history item
             // This maps to the selectedLlmModelAlias state
             setSelectedLlmModelAlias(value || "");
             continue; // Handled, skip general assignment
          }
  ...
  // Effect to initialize history load when component mounts or filters/sort change
  useEffect(() => {
    // console.log("Initializing history fetch..."); // Debugging line
    initializeHistory();
  }, [historyFilters, historySort, initializeHistory]); // Depend on filters and sort objects

  const loadMoreButtonRef = useRef(null);

  // Infinite scroll effect (Optional, but good for UX)
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isLoadingHistoryMore && hasMoreHistory) {
          loadMoreHistory();
        }
      },
      { threshold: 1.0 }
  ...
  }, [isLoadingHistoryMore, hasMoreHistory, loadMoreHistory]); // Depend on loading, more history flag, and load function

  return (
    <div className="container mx-auto py-8 px-4">
      <Tabs value={activeMainTab} onValueChange={setActiveMainTab} className="w-full">
        <TabsList>
          <TabsTrigger value="fetchContent">Fetch Content</TabsTrigger>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <FetchForm
                ...
                llmModelSelectComponent={fetchingEngine === 'crawl4ai' ? (
                  <LlmModelSelect
                    selectedModelAlias={selectedLlmModelAlias}
                    onModelSelect={handleLlmModelSelect}
                    label="Select LLM Model for Crawl4AI (if LLM strategy used)"
                  />
                ) : null}
              />
              ...
              {/* Advanced Fetch Options (conditionally rendered) */}
              {showAdvanced && (
                <AdvancedFetchOptions
                   setFormState={setFormState} // AdvancedFetchOptions uses setFormState directly for some complex updates
                   fetchingEngine={fetchingEngine}
              ...
              {mainFetchError && (
                <div className="text-destructive mt-4">Error: {mainFetchError}</div>
              )}
              ...
              {formState.result && (
                <div className="mt-4">
                  ...
                  {/* Result Content Viewer */}
                   <FetchedContentViewer result={formState.result} />
        ...
        <TabsContent value="fetchHistory">
           {/* Fetch History Table with filters and sort */}
           <Card>
            <CardHeader>
              <CardTitle>Fetch History ({totalHistoryItems})</CardTitle>
            </CardHeader>
            <CardContent>
               ...
               <div className="flex flex-wrap items-center gap-4 mb-4">
                 <div className="flex-grow">
                    ...
                    <Input
                      id="historySearch"
                      placeholder="Search by URL or content summary..."
                 ...
                 <div>
                   <Label htmlFor="historyFilterEngine">Filter by Engine</Label>
                    <Select
                       value={historyFilterEngine}
                       onValueChange={setHistoryFilterEngine}
                    >
                      <SelectTrigger id="historyFilterEngine" className="w-[180px]">
                  ...
                  <div>
                   <Label htmlFor="historyFilterStatus">Filter by Status</Label>
                    <Select
                       value={historyFilterStatus}
                       onValueChange={setHistoryFilterStatus}
                    >
                      <SelectTrigger id="historyFilterStatus" className="w-[180px]">
                  ...
                  {/* History Sort - Add this section */}
                 <div>
                   <Label htmlFor="historySortColumn">Sort By</Label>
                   <Select
                      value={historySortColumn}
                      onValueChange={setHistorySortColumn}
                   >
                     <SelectTrigger id="historySortColumn" className="w-[180px]">
                       <SelectValue placeholder="Sort By" />
                     </SelectTrigger>
                     <SelectContent>
                       <SelectItem value="fetch_date">Fetch Date</SelectItem>
                 ...
                 <div>
                   <Label htmlFor="historySortDirection">Order</Label>
                   <Select
                      value={historySortDirection}
                      onValueChange={setHistorySortDirection}
                   >
                     <SelectTrigger id="historySortDirection" className="w-[180px]">
              ...
              {isLoadingHistoryInitial ? (
                ...
                <div className="text-destructive text-center">Error loading history: {historyError.message}</div>
              ) : fetchHistoryItems && fetchHistoryItems.length > 0 ? (
                <>
                  <ScrollArea className="h-[500px] w-full rounded-md border">
                    <FetchHistoryTable items={fetchHistoryItems} onView={handleViewHistoryItem} onDelete={handleDeleteHistoryItem} onRefetch={handleRefetchHistoryItem} />
                  </ScrollArea>
                  ...
                  {hasMoreHistory && (
                    <div className="text-center mt-4">
                      <Button
                        ref={loadMoreButtonRef} // Attach ref for infinite scroll
                        onClick={loadMoreHistory}
                        disabled={isLoadingHistoryMore}
                      >
                        {isLoadingHistoryMore ? "Loading more..." : "Load More"}
                      </Button>
                    </div>
                  )}
                </>
              ) : (
                <div className="text-center">No fetch history found.</div>
              )}
            </CardContent>
           </Card>
        </TabsContent>
      ...
      {/* Hidden Viewers for History Items (when viewing history) */}
      {/* Render these conditionally or in a modal if preferred */}
       <Accordion type="single" collapsible className="w-full mt-8">
         {formState.result && activeMainTab === 'fetchHistory' && (
            <AccordionItem value="historyItemContent">
             <AccordionTrigger>View History Item Content</AccordionTrigger>
              <AccordionContent>
                 <FetchedContentViewer result={formState.result} />
              </AccordionContent>
            </AccordionItem>
