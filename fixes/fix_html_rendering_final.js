/**
 * Final version of the HTML rendering fix for PMOVES-transcribe-and-fetch
...
function formatAnalysisContent(content) {
  ...
  if (containsCssClassNames) {
    ...
    return finalProcessHtmlContent(content);
  }
...
/**
 * Final approach to process HTML content
 * This function uses a comprehensive approach to fix the HTML rendering issues
 */
function finalProcessHtmlContent(content) {
  // Step 1: Extract and store YouTube links for later processing
  const youtubeLinks = [];
  ...
  while ((youtubeMatch = youtubeRegex.exec(content)) !== null) {
    youtubeLinks.push({
      fullMatch: youtubeMatch[0],
      videoId: youtubeMatch[1],
      index: youtubeMatch.index
    });
  }
  
  // Step 2: Replace all CSS class references with actual HTML elements
  ...
  // Step 3: Fix list items with a more comprehensive approach
  ...
  // Step 4: Replace YouTube links with properly formatted links
  for (const link of youtubeLinks) {
    const formattedLink = \`<a href="https://www.youtube.com/watch?v=\${link.videoId}" target="_blank" rel="noopener noreferrer" class="text-blue-600 underline hover:no-underline flex items-center"><span class="mr-1">🎬</span>Watch Video</a>\`;
    content = content.replace(link.fullMatch, formattedLink);
  }
  
  // Step 5: Remove any remaining CSS class references
  ...
  // Step 6: Clean up any remaining quotes and formatting issues
  content = content.replace(/""/g, '');
  content = content.replace(/">/g, '>');
  
  // Step 7: Remove any remaining target/rel attributes that are displayed as text
  ...
  // Step 11: Fix any remaining issues with YouTube links
  ...
  // Step 12: Fix any remaining issues with the URL field
