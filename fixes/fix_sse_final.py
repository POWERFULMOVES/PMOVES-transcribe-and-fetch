...
def fix_sse_endpoint():
    """
    Create a completely new implementation of the SSE endpoint
    """
    main_py_path = os.path.join('backend', 'app', 'main.py')
    
    # Create a backup of the original file
    backup_path = main_py_path + '.bak.final_fix'
    ...
    start_idx = original_content.find(combined_updates_start)
    end_idx = original_content.find(combined_updates_end)
    ...
    correct_implementation = """@app.get("/combined-updates")
async def get_combined_updates(request: Request):
    ...
    new_content = original_content[:start_idx] + correct_implementation + "\n\n\n" + original_content[end_idx:]
    
    # Also fix the download-status endpoint
    download_status_start = "@app.get(\"/api/download-status\")"
    download_status_end = "@app.post(\"/api/video-info\")"
    
    ds_start_idx = new_content.find(download_status_start)
    ds_end_idx = new_content.find(download_status_end)
    
    if ds_start_idx != -1 and ds_end_idx != -1:
        # Find the problematic line in the download-status endpoint
        problematic_line = "yield f\"data: {json.dumps(update_data)}\\n\\n\"\\n\\n\""
        fixed_line = "yield f\"data: {json.dumps(update_data)}\\n\\n\""
        
        # Replace all occurrences of the problematic line
        download_status_content = new_content[ds_start_idx:ds_end_idx]
        fixed_download_status = download_status_content.replace(problematic_line, fixed_line)
        fixed_download_status = fixed_download_status.replace("\\n\\n\"\\n\\n\"", "\\n\\n\"")
        
        # Update the content
        new_content = new_content[:ds_start_idx] + fixed_download_status + new_content[ds_end_idx:]
    
    # Write the fixed content back to the file
    with open(main_py_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print(f"Fixed SSE endpoints in {main_py_path}")
