...
class ChatMessageListener:
    ...
    async def _process_message_simple(self, message_text: str, timestamp: datetime) -> str:
        """Processes message using LLM if available, otherwise echoes."""
        llm_response_text = None
        llm_service = self.agent_framework.get_llm_registry_service()

        if llm_service:
            chat_models = llm_service.get_available_models(capability_filter="chat_completion")
            if chat_models:
                ...
                logger.info(f"Attempting to use LLM '{selected_model.model_id}' for chat response.")
                ...
                try:
                    ...
                    # )
                    llm_response_text = await llm_service.get_chat_completion(selected_model.model_id, prompt_messages)
                    
                    if llm_response_text:
                        logger.info(f"LLM '{selected_model.model_id}' responded successfully.")
                    else:
                        logger.warning(f"LLM '{selected_model.model_id}' returned no content. Falling back to echo.")
                except Exception as e:
                    logger.error(f"Error during LLM chat completion with '{selected_model.model_id}': {e}", exc_info=True)
                    llm_response_text = None # Ensure fallback on error
            else:
                logger.warning("No chat-capable LLMs found in the registry. Falling back to echo.")
        else:
    ...
    async def _poll_new_messages(self):
        ...
        while self._running:
            try:
                ...
                response = await self.supabase_client.table(self.input_table_name)\
                    .select("id, message_text, timestamp, user_id, session_id")\
                    .eq("status", "new")\
                    .order("timestamp", desc=False)\
                    .limit(10) \
                    .execute() # Add limit to avoid processing too many at once

                if response.error:
                    logger.error(f"Error fetching new messages: {response.error.message}")
                ...
                for msg in new_messages:
                    ...
                    original_timestamp_str = msg["timestamp"] # Assuming it's ISO format string
                    
                    try:
                        original_timestamp = datetime.fromisoformat(original_timestamp_str)
                    except (ValueError, TypeError):
                        logger.warning(f"Could not parse timestamp '{original_timestamp_str}' for message {message_id}. Using current time.")
                        original_timestamp = datetime.now(timezone.utc)


                    # 2. Update status to "processing"
                    update_resp = await self.supabase_client.table(self.input_table_name)\
                        .update({"status": "processing", "updated_at": datetime.now(timezone.utc).isoformat()})\
                        .eq("id", message_id)\
                        .execute()
                    
                    if update_resp.error:
                        logger.error(f"Error updating message {message_id} to 'processing': {update_resp.error.message}")
                        continue # Skip this message, try again next poll

                    # 3. Process the message (simple echo)
                    response_text = await self._process_message_simple(original_message_text, original_timestamp)
                    ...
                    insert_resp = await self.supabase_client.table(self.output_table_name)\
                        .insert(insert_resp_payload)\
                        .execute()

                    if insert_resp.error:
                        logger.error(f"Error inserting agent response for message {message_id}: {insert_resp.error.message}")
                        ...
                        await self.supabase_client.table(self.input_table_name)\
                            .update({"status": "failed_response_insertion", "updated_at": datetime.now(timezone.utc).isoformat()})\
                            .eq("id", message_id)\
                            .execute()
                        continue 

                    # 5. Update original message's status to "processed"
                    final_update_resp = await self.supabase_client.table(self.input_table_name)\
                        .update({"status": "processed", "updated_at": datetime.now(timezone.utc).isoformat()})\
                        .eq("id", message_id)\
                        .execute()

                    if final_update_resp.error:
                        logger.error(f"Error updating message {message_id} to 'processed': {final_update_resp.error.message}")
                        # This is less critical as response was already sent.
                    
                    logger.info(f"Successfully processed and responded to message ID {message_id}.")
    ...
    async def stop(self):
        if self._running and self._polling_task:
            ...
            try:
                ...
                logger.error(f"Error during polling task cancellation: {e}", exc_info=True)
