...
from typing import Union, List as PyList # Renamed to PyList to avoid conflict with Pydantic List
...
if not SUPABASE_URL or not SUPABASE_KEY:
    ...
    db_client: Optional[Client] = None # Renamed for clarity from supabase_client to avoid conflict with module name
else:
    try:
...
AGENT_ENDPOINT_URL = os.getenv("AGENT_ENDPOINT_URL", f"http://localhost:{os.getenv('AGENT_PORT', 8002)}")
...
def parse_json_env_var(var_name: str, default_value: Any) -> Any:
    ...
    if var_value:
        try:
            ...
            logger.warning(f"Failed to parse JSON from env var {var_name}. Using default.")
...
agent_framework = AgentFramework(
    ...
    capabilities=parse_json_env_var("AGENT_CAPABILITIES", ["supabase_interaction", "query_execution"]),
    input_schema=parse_json_env_var("AGENT_INPUT_SCHEMA", {"type": "object", "properties": {}}),
    output_schema=parse_json_env_var("AGENT_OUTPUT_SCHEMA", {"type": "object", "properties": {}}),
...
if db_client: # Only initialize if Supabase client is available
    ...
    logger.warning("Supabase client not available. ChatMessageListener will not be started.")
...
async def startup_event():
    ...
    if db_client: # Use the renamed client
        try:
            ...
            logger.error(f"Supabase connection test or chat listener start failed during startup: {e}")
    else:
        logger.warning("Supabase client (db_client) not initialized. Some functionalities might be limited.")
...
class UpsertDataRequest(BaseModel):
    ...
    data: Union[Dict[str, Any], PyList[Dict[str, Any]]] = Field(..., description="A single data record (dict) or a list of records to upsert.")
    conflict_on: Optional[str] = Field(None, description="Optional column name for conflict resolution (e.g., primary key). If provided, performs an upsert. If None, performs an insert.")
...
class ColumnSchema(BaseModel):
    ...
    type: str = Field(..., description="Column data type (e.g., TEXT, INTEGER, TIMESTAMPTZ DEFAULT NOW()).")

class TableSchemaRequest(BaseModel):
    columns: PyList[ColumnSchema]

class ManageTableRequest(BaseModel):
    operation: str = Field(..., description="Operation to perform: create_table, delete_table, add_column, drop_column.")
    table_name: str = Field(..., description="Name of the table.")
    schema: Optional[TableSchemaRequest] = Field(None, description="Schema definition for 'create_table'.")
    column_definition: Optional[ColumnSchema] = Field(None, description="Definition for 'add_column'.")
    column_name: Optional[str] = Field(None, description="Name of the column for 'drop_column'.")
    ...
    def check_column_def_for_add(cls, v, values):
        if values.get('operation') == 'add_column' and v is None:
            raise ValueError("Column definition must be provided for 'add_column' operation.")
        return v
    ...
    def check_column_name_for_drop(cls, v, values):
        if values.get('operation') == 'drop_column' and v is None:
            raise ValueError("Column name must be provided for 'drop_column' operation.")
        return v
...
class StreamQueryDetails(BaseModel):
    ...
    filters: Optional[PyList[PyList[Any]]] = Field(None, description="List of filters, e.g., [['status', 'eq', 'active']].")
    order_by: Optional[str] = Field(None, description="Column to order by, e.g., 'created_at.desc'.")
    # Limit is handled by chunk_size in streaming logic, but could be an overall cap.

class StreamResultsQueryParams(BaseModel):
    table_name: str
    chunk_size: int = Field(50, gt=0, le=1000, description="Number of records to fetch per chunk.")
    query_params: Optional[StreamQueryDetails] = None # Embedded model for query details
...
class InfiniteQueryParams(BaseModel):
    table_name: str
    select_params: str = Field("*", description="Columns to select, e.g., '*' or 'id,name,status'.")
...
@app.post("/adjust-params", response_model=AdjustParamsResponse, summary="Adjust Agent's Internal Parameters")
async def adjust_params_handler(payload: AdjustParamsRequest): # No db client needed here
    ...
    if "allow_table_management_ddl" in payload.params:
        logger.info(f"Security relevant parameter 'allow_table_management_ddl' changed to: {agent_internal_settings['allow_table_management_ddl']}")
...
@app.post("/upsert-data", response_model=UpsertDataResponse, summary="Upsert Data into a Supabase Table")
async def upsert_data_handler(payload: UpsertDataRequest, db: Client = Depends(get_supabase_client)): # db refers to db_client
    logger.info(f"Received request to upsert data into table '{payload.table_name}'. Data (first 50 chars if list): {str(payload.data)[:50 if isinstance(payload.data, list) else 500]}")
    try:
        # The upsert method in supabase-py takes a list of dicts.
        # If a single dict is provided, wrap it in a list.
        data_to_upsert = [payload.data] if isinstance(payload.data, dict) else payload.data
        
        query = db.table(payload.table_name).upsert(
            data_to_upsert, 
            on_conflict=payload.conflict_on if payload.conflict_on else None # Pass None if not specified for simple insert
        )
        ...
        if response.data:
            ...
            if hasattr(response, 'error') and response.error:
                 logger.error(f"Supabase upsert error for table '{payload.table_name}': {response.error.message}")
    ...
    except HTTPException: # Re-raise HTTPExceptions from get_supabase_client or validation
        raise
    except Exception as e: # Catching generic supabase client errors or other unexpected errors
        logger.error(f"Error during upsert operation for table '{payload.table_name}': {e}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to upsert data: {error_detail}")


@app.post("/manage-table", response_model=ManageTableResponse, summary="Manage Supabase Tables (DDL Operations - Use with Extreme Caution)")
async def manage_table_handler(payload: ManageTableRequest, db: Client = Depends(get_supabase_client)):
    logger.warning(f"Received DDL request for table '{payload.table_name}' with operation '{payload.operation}'.")
    
    if not agent_internal_settings.get("allow_table_management_ddl", False):
        logger.error("DDL operations are disabled by agent configuration ('allow_table_management_ddl' is False).")
        raise HTTPException(
            status_code=403, 
    ...
    if payload.operation == "create_table":
        if not payload.schema or not payload.schema.columns:
            raise HTTPException(status_code=400, detail="Schema with columns must be provided for 'create_table'.")
        cols_defs = [f"{col.name} {col.type}" for col in payload.schema.columns]
        sql_statement = f"CREATE TABLE IF NOT EXISTS {payload.table_name} ({', '.join(cols_defs)});"
        ...
        if not payload.column_definition:
            raise HTTPException(status_code=400, detail="Column definition must be provided for 'add_column'.")
        ...
        if not payload.column_name:
            raise HTTPException(status_code=400, detail="Column name must be provided for 'drop_column'.")
        ...
        raise HTTPException(status_code=400, detail=f"Unsupported table management operation: {payload.operation}")

    if not sql_statement:
        raise HTTPException(status_code=500, detail="Failed to construct SQL statement for the operation.")
    ...
    try:
        ...
        logger.error(f"Error during table management operation for table '{payload.table_name}': {e}")
        error_detail = str(e)
        if hasattr(e, 'message'):
            error_detail = e.message
        raise HTTPException(status_code=500, detail=f"Table management operation failed: {error_detail}")
...
async def build_supabase_query(db: Client, table_name: str, query_details: Optional[StreamQueryDetails] = None, page: Optional[int] = None, page_size: Optional[int] = None):
    """
    ...
    query = query.select(select_columns, count="exact" if page is not None else None) # Request count only for pagination
...
@app.post("/stream-results", summary="Stream Query Results from Supabase (POST with JSON body)")
async def stream_results_handler(params: StreamResultsQueryParams, db: Client = Depends(get_supabase_client)):
    """
    ...
    """
    logger.info(f"Streaming request for table '{params.table_name}' with chunk_size {params.chunk_size} and query: {params.query_params}")

    async def streamer(table_name: str, chunk_size: int, query_details: Optional[StreamQueryDetails]):
        current_page = 0 # Supabase pages are effectively 0-indexed for range logic, or use offset
        records_fetched_in_current_chunk = chunk_size # Initialize to enter loop

        while records_fetched_in_current_chunk == chunk_size: # Continue if last fetch was a full chunk
            ...
            try:
                ...
                select_cols = query_details.select if query_details and query_details.select else "*"
                ...
                if query_details and query_details.filters:
                    for f_item in query_details.filters:
                        if len(f_item) == 3:
                            ...
                            logger.warning(f"Skipping invalid filter item in stream: {f_item}")
                ...
                logger.debug(f"Executing stream chunk query for table {table_name}: page {current_page+1}, range {offset_start}-{offset_end}")
                ...
                if response.data:
                    ...
                    if records_fetched_in_current_chunk < chunk_size:
                        logger.info(f"Stream finished for table {table_name}: fetched last chunk of size {records_fetched_in_current_chunk}.")
                        break # Last chunk was smaller than chunk_size, so no more data
                else:
                    logger.info(f"Stream finished for table {table_name}: no data in current chunk.")
                    break # No data returned, end of stream

                current_page += 1
                await asyncio.sleep(0.01) # Small sleep to prevent tight loop if connection is very fast / allow context switching

            except Exception as e:
                logger.error(f"Error during data streaming for table {table_name}: {e}")
                error_obj = {"error": "Failed to fetch or stream data.", "details": str(e)}
...
@app.post("/infinite-query", response_model=InfiniteQueryResponse, summary="Execute a Paginated Query (POST with JSON body)")
async def infinite_query_handler(params: InfiniteQueryParams, db: Client = Depends(get_supabase_client)):
    """
    Supports 'infinite scroll' type queries or fetching paginated data.
    Query parameters are passed in the request body as JSON.
    """
    logger.info(f"Infinite query request for table '{params.table_name}' with params: {params.dict(exclude_none=True)}")
    ...
    try:
        ...
        if params.order_params:
            query = query.order(params.order_params.field, desc=not params.order_params.ascending)
        
        offset_start = (params.page - 1) * params.page_size
        offset_end = params.page * params.page_size - 1 # Supabase range is inclusive
        query = query.range(offset_start, offset_end)
        
        logger.debug(f"Executing paginated query for table {params.table_name}: page {params.page}, size {params.page_size}")
        response = await query.execute()

        total_records = response.count if response.count is not None else 0
        total_pages = (total_records + params.page_size - 1) // params.page_size if total_records is not None else None
        ...
        if response.data is not None: # data can be [] which is valid
            ...
            logger.warning(f"No data field in response for infinite query on table {params.table_name}, though no error raised.")
            ...
            raise HTTPException(status_code=500, detail="Query executed but no data was returned in the expected format.")

    except HTTPException: # Re-raise
        raise
    except Exception as e:
        logger.error(f"Error during paginated query for table '{params.table_name}': {e}")
        error_detail = str(e)
        if hasattr(e, 'message'): # PostgrestError
            error_detail = e.message
        raise HTTPException(status_code=500, detail=f"Failed to execute paginated query: {error_detail}")
