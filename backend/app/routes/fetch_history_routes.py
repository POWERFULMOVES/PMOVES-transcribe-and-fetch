...
try:
    ...
    logger.info(
        f"Ensured fetched content storage directory exists: {CONTENT_STORAGE_BASE_DIR}"
    )
except OSError as e:
    logger.error(
        f"CRITICAL: Could not create fetched content storage directory {CONTENT_STORAGE_BASE_DIR}: {e}. Content retrieval will fail.",
        exc_info=True,
    )
...
def secure_join(base: Path, filename: str) -> Optional[Path]:
    ...
    if (
        full_path.parent == resolved_base or full_path == resolved_base
    ):  # full_path could be the base itself if filename is empty or "."
        return full_path

    logger.warning(
        f"Path traversal attempt blocked or invalid path. Base: '{resolved_base}', Filename: '{filename}', Resolved Full: '{full_path}'"
    )
    return None
...
async def get_fetch_history_item_content(
    history_id: uuid.UUID, supabase_client: Client = Depends(get_client)
):
    """
    ...
    try:
        response = await asyncio.to_thread(
            ...
            .select(
                "id, content_storage_path, output_type, title, url, supabase_storage_path"
            )  # Added supabase_storage_path
        ...
        if hasattr(response, "error") and response.error:
            logger.error(
                f"Supabase error fetching history item {history_id}: {response.error}"
            )
            raise HTTPException(
        ...
        if not hasattr(response, "data") or not response.data:
            ...
            raise HTTPException(
                ...
                detail="Fetch history item not found.",
            )
        ...
        if output_type_value is None:
            output_type = "unknown"
            logger.warning(
                f"output_type is null for history_id {history_id}. Defaulting to 'unknown'."
            )
        else:
        ...
        local_content_path_str = item_data.get(
            "content_storage_path"
        )  # Path to local file, might be primary or fallback
        ...
        if db_item_supabase_path:
            logger.info(
                f"Supabase path found for {history_id}: {db_item_supabase_path}. Attempting Supabase download."
            )
            try:
                file_bytes = await asyncio.to_thread(
                    supabase_client.storage.from_(BUCKET_NAME).download,
                    path=db_item_supabase_path,
                )
                logger.info(
                    f"Successfully downloaded content from Supabase for {history_id} from path {db_item_supabase_path}"
                )
                ...
                if output_type in [
                    ...
                    try:
                        ...
                        if signed_url:
                            response_payload_base["pdf_supabase_signed_url"] = (
                                signed_url
                            )
                            response_payload_base["markdown_content"] = (
                                None  # No markdown for PDF
                            )
                            logger.info(
                                f"Generated signed URL for Supabase PDF: {history_id}"
                            )
                            return JSONResponse(content=response_payload_base)
                        else:
                            logger.error(
                                f"Failed to get signed URL from Supabase response for {db_item_supabase_path}. Response: {signed_url_response}"
                            )
                            # Fall through to local file logic if signed URL fails
                    except Exception as e_signed_url:
                        logger.error(
                            f"Error generating signed URL for Supabase PDF {db_item_supabase_path}: {e_signed_url}",
                            exc_info=True,
                        )
                ...
                logger.error(
                    f"Failed to download content from Supabase path {db_item_supabase_path} for {history_id}: {e_supabase_dl}. Falling back to local file system.",
                    exc_info=True,
                )
        ...
        logger.info(
            f"Proceeding with local file system retrieval for {history_id}. Local path: {local_content_path_str}"
        )

        if not local_content_path_str:  # Check if local path is also missing
            logger.warning(
                f"No local content_storage_path found for history item {history_id} after Supabase check/failure."
            )
            return JSONResponse(
                content={
        ...
        full_local_content_path = (
            CONTENT_STORAGE_BASE_DIR / local_content_path_str
        ).resolve()

        # Security check for local path
        if not str(full_local_content_path).startswith(
            str(CONTENT_STORAGE_BASE_DIR.resolve())
        ):
            logger.error(
                f"Security check failed for local path: {full_local_content_path}. Original path from DB: '{local_content_path_str}'"
            )
            # This indicates a potentially compromised local_content_path_str in DB.
            raise HTTPException(
                status_code=403,
                detail="Access to local content path denied due to security policy.",
            )

        if not await asyncio.to_thread(full_local_content_path.is_file):
            logger.error(
                f"Local content file not found at path: {full_local_content_path} (from DB local_content_path: {local_content_path_str}) for history item {history_id}."
            )
            return JSONResponse(
                content={
        ...
        logger.info(
            f"Attempting to serve file from local filesystem: {full_local_content_path} with type: {output_type}"
        )
        ...
        if output_type in [
            ...
            try:
                async with aiofiles.open(
                    full_local_content_path, mode="r", encoding="utf-8"
                ) as f:
                ...
                logger.error(
                    f"Error reading local text file {full_local_content_path} for history item {history_id}: {e}",
                    exc_info=True,
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Error reading local content file: {e}",
                )
            ...
            logger.warning(
                f"Unsupported output_type '{output_type}' for local history item {history_id} at path {full_local_content_path}."
            )
            response_payload_base_local["message"] = (
                f"Unsupported content type '{output_type}'. Path provided."
            )
            response_payload_base_local["file_path"] = (
                local_content_path_str  # Generic file path
            )
            return JSONResponse(content=response_payload_base_local)
        ...
        logger.error(
            f"Unexpected error retrieving content for history item {history_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post(
    "/",  # Adjusted path to be relative to the prefix in main.py
...
async def create_fetch_history_item(
    item: FetchHistoryCreate, supabase_client: Client = Depends(get_client)
):
    """
    Creates a new entry in the fetch_history table.
    This endpoint is now designed to be called internally or by trusted services
    to record the outcome of a fetch operation.
    """
    if not supabase_client:
        logger.error("Supabase client not available for creating fetch history.")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
    ...
    if item.upload_to_supabase:
        if not item.content_storage_path:
            logger.warning(
                f"content_storage_path is required for Supabase upload attempt for URL: {item.url}"
            )
            raise HTTPException(
                status_code=400,
                detail="content_storage_path is required for Supabase upload.",
            )

        full_local_path = (
            CONTENT_STORAGE_BASE_DIR / item.content_storage_path
        ).resolve()
        ...
        if not str(full_local_path).startswith(str(CONTENT_STORAGE_BASE_DIR.resolve())):
            logger.error(
                f"Security check failed for local path for Supabase upload: {full_local_path}. Original path: {item.content_storage_path}"
            )
            raise HTTPException(
                status_code=400,
                detail="Invalid content_storage_path leading to potential path traversal.",
            )

        if not await asyncio.to_thread(full_local_path.is_file):
            logger.error(
                f"Local content file not found for Supabase upload: {full_local_path}"
            )
            raise HTTPException(
                status_code=404,
                detail=f"Local content file not found: {item.content_storage_path}",
            )
        ...
        storage_path_in_bucket = (
            f"{item.user_id or 'public'}/{history_id}/{filename_in_bucket}"
        )
        ...
        try:
            ...
            elif item.output_type == "html":
                upload_content_type = "text/html"
            elif item.output_type == "json":
                upload_content_type = "application/json"
            elif item.output_type == "pdf":
                upload_content_type = "application/pdf"
            elif item.output_type == "text" or item.output_type == "txt":
                upload_content_type = "text/plain"
            # Add more mappings if needed

            logger.info(
                f"Attempting to upload to Supabase: bucket='{BUCKET_NAME}', path='{storage_path_in_bucket}', content_type='{upload_content_type}'"
            )
            ...
            supabase_file_storage_path = (
                storage_path_in_bucket  # Store the bucket path, not full URL
            )
            logger.info(
                f"Successfully uploaded {item.content_storage_path} to Supabase bucket {BUCKET_NAME} at {storage_path_in_bucket}"
            )
            ...
            if hasattr(e, "message"):
                supa_error_message = e.message

            logger.error(
                f"Failed to upload {item.content_storage_path} to Supabase Storage. Path: {storage_path_in_bucket}. Error: {supa_error_message}",
                exc_info=True,
            )
    ...
    try:
        ...
        if insert_response.data:
            logger.info(
                f"Fetch history item created with ID: {history_id} for URL: {item.url}"
            )
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to create history item or no data returned from database.",
            )
        ...
        logger.error(
            f"Unexpected error creating fetch history item in DB for URL {item.url}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected error during database operation: {str(e)}",
        )
...
async def delete_fetch_history_item(
    history_id: uuid.UUID, supabase_client: Client = Depends(get_client)
):
    """
    Deletes a fetch history record from the database and its associated content
    from both local storage and Supabase storage.
    """
    ...
    try:
        ...
        select_response = await asyncio.to_thread(
            supabase_client.table("fetch_history")
            .select(
                "id, content_storage_path, supabase_storage_path, output_type"
            )  # Added supabase_storage_path
            .eq("id", str(history_id))
            .single()
            .execute
        ...
        if hasattr(select_response, "error") and select_response.error:
            logger.error(
                f"Supabase error fetching history item {history_id} for deletion: {select_response.error.message if hasattr(select_response.error, 'message') else select_response.error}"
            )
            if "PGRST116" in str(
                getattr(select_response.error, "message", "")
            ):  # Resource not found
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="Fetch history item not found.",
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Database error fetching item: {getattr(select_response.error, 'message', 'Unknown DB error')}",
            )

        if not hasattr(select_response, "data") or not select_response.data:
            logger.warning(
                f"Fetch history item with ID {history_id} not found for deletion."
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Fetch history item not found.",
        ...
        if hasattr(delete_db_response, "error") and delete_db_response.error:
            logger.error(
                f"Supabase error deleting history item {history_id} from DB: {delete_db_response.error.message if hasattr(delete_db_response.error, 'message') else delete_db_response.error}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Database error deleting item: {getattr(delete_db_response.error, 'message', 'Unknown DB error')}",
            )
        ...
        if not hasattr(delete_db_response, "data") or not delete_db_response.data:
            ...
            logger.warning(
                f"No rows deleted from DB for history item ID {history_id}. It might have been deleted by another process."
            )
            # Proceed to file deletion attempts anyway, as files might be orphaned.
        else:
            logger.info(
                f"Successfully deleted record for history item ID: {history_id} from database."
            )
        ...
        if db_item_supabase_path:
            ...
            try:
                logger.info(
                    f"Attempting to delete from Supabase Storage: bucket='{BUCKET_NAME}', path='{db_item_supabase_path}' for history item {history_id}"
                )
                ...
                # else:
                logger.info(
                    f"Deletion initiated for {db_item_supabase_path} from Supabase Storage for {history_id}. Response: {remove_storage_response}"
                )

            except Exception as e_supa_delete:
                logger.error(
                    f"Error during Supabase Storage deletion of {db_item_supabase_path} for history item {history_id}: {e_supa_delete}",
                    exc_info=True,
                )
        ...
        if local_content_path_str:
            ...
            if not str(full_local_content_path).startswith(
                ...
                logger.error(
                    f"Security check failed for local file deletion: Path '{full_local_content_path}' is outside of base '{CONTENT_STORAGE_BASE_DIR.resolve()}'. Original path: '{local_content_path_str}' for item {history_id}."
                )
                ...
                try:
                    if await asyncio.to_thread(full_local_content_path.is_file):
                        ...
                        logger.info(
                            f"Successfully deleted local content file: {full_local_content_path} for history item {history_id}."
                        )
                    else:
                        logger.warning(
                            f"Local content file not found at {full_local_content_path} for history item {history_id} during deletion. It might have been already deleted."
                        )
                except OSError as e_os_remove:
                    logger.error(
                        f"Error deleting local content file {full_local_content_path} for history item {history_id}: {e_os_remove}",
                        exc_info=True,
                    )
                    # Do not re-raise.
        else:
            logger.info(
                f"No local content_storage_path found for history item {history_id}, so no local file to delete."
            )
        ...
        logger.error(
            f"Unexpected error deleting history item {history_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
