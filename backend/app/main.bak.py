...
# Local imports - Content and File Management
...
# Local imports - Video Processing
...
# Local imports - Search and Frontend
...
async def vector_search_stream(
    ...
    async def event_generator():
        try:
            ...
            yield adapter.create_sse_message("Connected to search stream", "log")
            
            # Update search parameters
            search_params.update_params('fine_grained',
                similarity_threshold=fine_grained_similarity_threshold,
                content_weight=fine_grained_content_weight,
                result_percentage=fine_grained_result_percentage
            )
            search_params.update_params('contextual',
                similarity_threshold=contextual_similarity_threshold,
                content_weight=contextual_content_weight,
                result_percentage=contextual_result_percentage
            )
            search_params.update_params('overview',
                similarity_threshold=overview_similarity_threshold,
                content_weight=overview_content_weight,
                result_percentage=overview_result_percentage
            )

            # Log query details
            yield adapter.create_sse_message(f"Generated embedding for query: {query}", "log")
            yield adapter.create_sse_message(
                f"Query tokens: {token_counter.count_embedding_tokens(query)}", "log"
            )

            # Perform searches
            # Hybrid Search
            yield adapter.create_sse_message("Running hybrid search...", "log")
            hybrid_results = advanced_hybrid_search(query, limit=10, content_weight_override=0.7)
            formatted_hybrid = adapter.format_search_results(hybrid_results, 'hybrid')
            yield adapter.create_sse_message(formatted_hybrid, "results")
            yield adapter.create_sse_message(f"Hybrid search returned {len(hybrid_results)} results", "log")

            # Dot Product Search
            yield adapter.create_sse_message("Running dot product search...", "log")
            dot_results = dot_product_search(query, limit=10)
            formatted_dot = adapter.format_search_results(dot_results, 'dot_product')
            yield adapter.create_sse_message(formatted_dot, "results")
            yield adapter.create_sse_message(f"Dot product search returned {len(dot_results)} results", "log")

            # Keyword Search
            yield adapter.create_sse_message("Running keyword search...", "log")
            keyword_results = keyword_search(query, limit=10)
            formatted_keyword = adapter.format_search_results(keyword_results, 'keyword')
            yield adapter.create_sse_message(formatted_keyword, "results")
            yield adapter.create_sse_message(f"Keyword search returned {len(keyword_results)} results", "log")

            # Send token usage
            token_usage = adapter.format_token_usage(token_counter)
            yield adapter.create_sse_message(token_usage, "token_usage")

            # Generate AI analysis
            yield adapter.create_sse_message("Generating AI analysis...", "log")
            all_results = hybrid_results + dot_results + keyword_results
            
            # Get both Groq and OpenAI analyses
            analyses = analyze_search_results(all_results, provider='groq')
            
            # Track if we got any AI responses
            got_ai_response = False
            
            # Send Groq response if available
            if 'groq' in analyses:
                yield adapter.create_sse_message(analyses['groq'], "ai_response_groq")
                got_ai_response = True
            
            # Send OpenAI response if available
            if 'openai' in analyses:
                yield adapter.create_sse_message(analyses['openai'], "ai_response_openai")
                got_ai_response = True
            
            # If there was an error, send it
            if 'error' in analyses:
                yield adapter.create_sse_message(analyses['error'], "error")
            elif not got_ai_response:
                yield adapter.create_sse_message("No AI analysis was generated", "error")

            # Send completion message
            yield adapter.create_sse_message(None, "complete")

        except Exception as e:
            print(f"Search error: {str(e)}")
            yield adapter.create_sse_message(str(e), "error")
