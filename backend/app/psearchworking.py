...
try:
    from .app_config import (
        DEFAULT_ANALYSIS_MODEL_ID as _DEFAULT_ANALYSIS_MODEL_ID_imported,
    )
    ...
    logger.info(
        "Successfully imported from LLM Registry and app_config for psearchworking."
    )
except ImportError as e:
    logger.error(
        f"Failed to import from LLM Registry or app_config in psearchworking: {e}. Analysis via registry will be disabled or use fallbacks."
    )
...
SCORE_STYLES = {
    "high": {"threshold": 0.8, "style": "bold green", "description": "high relevance"},
    "medium": {
        "threshold": 0.6,
        "style": "bold yellow",
        "description": "medium relevance",
    },
    "low": {"threshold": 0.0, "style": "bold red", "description": "low relevance"},
    "default": {"style": "dim", "description": "N/A"},
}

# Table Styling
TABLE_STYLES = {
    "title": {"style": "bold cyan", "icon": "📊"},
    "header": {"style": "bold magenta"},
    "border": {"style": "blue"},
    "columns": {
        "score": {"style": None, "width": 8, "justify": "right"},
        "method": {"style": "cyan", "width": 15},
        "source": {"style": "yellow", "width": 20},
        "content": {"style": "white", "width": 40, "overflow": "fold"},
        "video_id": {"style": "yellow", "width": 15},
        "segment_id": {"style": "yellow", "width": 10},
        "metadata": {"style": "blue", "width": 20, "overflow": "fold"},
        "start_time": {"style": "cyan", "width": 10},
        "end_time": {"style": "cyan", "width": 10},
        "watch_url": {"style": "blue", "width": 100, "overflow": "fold"},
    },
}

# Status and Progress Indicators
STATUS_INDICATORS = {"success": "✓", "error": "❌", "progress": "⏳", "pointer": "👉"}
...
PROGRESS_STAGES = {
    ...
    "search": {"icon": "🔍", "style": "bold yellow", "message": "Executing Search"},
    "filter": {"icon": "🔍", "style": "bold blue", "message": "Filtering Results"},
    "combine": {"icon": "📊", "style": "bold magenta", "message": "Combining Results"},
    "analyze": {"icon": "🤖", "style": "bold green", "message": "Analyzing Results"},
    "complete": {"icon": "✅", "style": "bold green", "message": "Operation Complete"},
}
...
class SearchResult:
    ...
    search_method: Optional[str] = (
        None  # Method used for search (keyword, dot_product, hybrid)
    )
    content_type: Optional[str] = (
        None  # Type of content (transcript, webpage, text, video, audio)
    )
    ...
    def from_db_result(cls, result: Dict[str, Any]) -> "SearchResult":
        ...
        if source == "video_transcriptions":
            ...
            if content:
                ...
                content = next(
                    (s for s in segments if s.strip() and not s.startswith("Segment")),
                    content,
                )
            metadata = {
            ...
            segment_id = result.get("segment_id")
            start_time = (
                result.get("start_time") if result.get("start_time") else "FULL"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result_id = (
            f"{self.content_id}_{self.segment_id or '0'}_{self.start_time or '0'}"
        )
        ...
        return {
            ...
            "has_context": bool(
                self.metadata
                and (
                    self.metadata.get("context_before")
                    or self.metadata.get("context_after")
                )
            ),
            "word_count": len(self.content.split()) if self.content else 0,
            "duration": f"{self.start_time or '0'} - {self.end_time or '0'}"
...
class SearchParameters:
    ...
    def load_current(self):
        ...
        logger.debug("load_current() called - parameters already loaded in memory")
        return True
        ...
        if tier in self.params:
            ...
            for key, value in kwargs.items():
                if key in self.params[tier]:
                    ...
                    logger.info(
                        f"  - {tier}.{key}: {old_value:.2f} → {float(value):.2f}"
                    )
                    ...
                    if key == "content_weight":
                        ...
                        logger.info(
                            f"  - {tier}.summary_weight updated to: {self.params[tier]['summary_weight']:.2f}"
                        )
            ...
            for tier in self.params:
                for param, value in self.params[tier].items():
                    if tier in old_params and param in old_params[tier]:
                        ...
                        if old_value != value:
                            logger.info(
                                f"  - {tier}.{param}: {old_value:.2f} → {value:.2f}"
                            )
    ...
    def update_from_frontend(
        ...
        if preset:
            if self.load_preset(preset):
                ...
                logger.warning(
                    f"Failed to load preset '{preset}' from frontend request"
                )

        # Create update dictionaries for each tier with only provided parameters
        updates = {"fine_grained": {}, "contextual": {}, "overview": {}}

        # Add parameters if they are provided (not None)
        if fine_grained_similarity_threshold is not None:
            updates["fine_grained"]["similarity_threshold"] = (
                fine_grained_similarity_threshold
            )
        ...
        if fine_grained_result_percentage is not None:
            updates["fine_grained"]["result_percentage"] = (
                fine_grained_result_percentage
            )
        ...
        if contextual_similarity_threshold is not None:
            updates["contextual"]["similarity_threshold"] = (
...
class TokenCounter:
    ...
    def __init__(self):
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not initialize encoders: {str(e)}[/yellow]"
            )
    ...
    def count_generation_tokens(self, input_text: str, output_text: str = None) -> dict:
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not count generation tokens: {str(e)}[/yellow]"
            )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            + sum(self.generation_tokens.values()),
        }
...
class ModelSelector:
    ...
    def generate_analysis(
        text: str,
        provider: str = "openai",
        analysis_model_id_override: Optional[str] = None,
    ) -> str:  # Added analysis_model_id_override
        ...
        if not chosen_model_id:
            ...
            if provider == "openai":
                chosen_model_id = (
                    DEFAULT_ANALYSIS_MODEL_ID  # Or a specific OpenAI model from config
                )
            elif provider == "groq":
                ...
                chosen_model_id = os.getenv(
                    "DEFAULT_GROQ_ANALYSIS_MODEL_ID", "groq/llama3-70b-8192"
                )  # Example
            else:
                logger.error(
                    f"Unsupported provider '{provider}' for analysis without a model_id_override."
                )
...
Summary of findings and recommendations"""
        ...
        try:
            ...
            max_tokens = (
                120000 if provider == "openai" else 5000
            )  # Conservative limits for OpenAI and Groq
            ...
            # Log token estimate
            console.print(
                f"[yellow]Estimated token count for {provider} analysis request: {estimated_tokens:.0f}[/yellow]"
            )
            ...
            if estimated_tokens > max_tokens:
                ...
                ) * 0.9  # 90% of the limit to be safe
                ...
                text = (
                    text[:text_chars_to_keep]
                    + "\n...[Content truncated due to token limits]"
                )
                console.print(
                    f"[yellow]Content truncated to fit within token limits. Keeping approximately {text_chars_to_keep} characters.[/yellow]"
                )
            ...
            if LLM_REGISTRY_AVAILABLE:  # Check if registry is available
                try:
                    ...
                    token_counter.count_generation_tokens(
                        "", output_text
                    )  # Count output
                    return output_text
                except Exception as e:
                    error_msg = str(e)
                    logger.error(
                        f"Error during analysis with model {chosen_model_id} via registry: {error_msg}",
                        exc_info=True,
                    )
                    ...
                    if (
                        ...
                        or "request too large" in error_msg.lower()
                    ):
                        console.print(
                            f"[red]Context length/token limit exceeded for model {chosen_model_id}. Try reducing search results.[/red]"
                        )
                        ...
                        console.print(
                            f"[red]Rate limit exceeded for model {chosen_model_id}.[/red]"
                        )
                        return (
                            f"Error: Rate limit exceeded for model {chosen_model_id}."
                        )
                    else:
                        console.print(
                            f"[red]Error generating analysis with {chosen_model_id} via registry: {error_msg}[/red]"
                        )
                ...
                logger.warning(
                    "LLM Registry not available, falling back to direct client calls for analysis."
                )
                if provider == "openai":
                    try:
                        response = openai_client.chat.completions.create(
                            model=ModelSelector.get_chat_model(
                                "openai"
                            ),  # Uses old static model
                            messages=[
                                {"role": "system", "content": system_prompt},
                                {"role": "user", "content": text},
                            ],
                        ...
                        if (
                            ...
                            console.print(
                                f"[red]OpenAI context length exceeded. Try reducing the number of search results.[/red]"
                            )
                    ...
                    try:
                        ...
                        if (
                            ...
                            console.print(
                                f"[red]Groq token limit exceeded. Try reducing the number of search results.[/red]"
                            )
            ...
            console.print(
                f"[red]Error generating analysis with {provider} (model: {chosen_model_id}): {str(e)}[/]"
            )
...
def analyze_search_results(
    ...
    if len(sorted_results) > max_results:
        logger.info(
            f"Limiting analysis to top {max_results} results (from {len(sorted_results)} total)"
        )
    ...
    return ModelSelector.generate_analysis(
        text, provider, analysis_model_id_override=analysis_model_id
    )
    ...
    if format_choice == "markdown":
        ...
        for method in ["keyword", "dot_product", "hybrid"]:
            ...
            if method_results:
                ...
                method_style = SEARCH_METHOD_STYLES.get(
                    method, SEARCH_METHOD_STYLES["default"]
                )
                ...
                for result in method_results:
                    try:
                        ...
                        result_method_style = SEARCH_METHOD_STYLES.get(
                            result.search_method, SEARCH_METHOD_STYLES["default"]
                        )
                        method_str = (
                            f"{result_method_style['icon']} {result.search_method}"
                        )
                        # Get source style safely
                        source_style = SOURCE_STYLES.get(
                            result.source, SOURCE_STYLES["default"]
                        )
                        ...
                        console.print(
                            f"[yellow]Warning: Could not format result: {str(e)}[/yellow]"
                        )
                        continue
        ...
        for result in sorted(results, key=lambda x: x.similarity, reverse=True):
            try:
                ...
                content = (
                    str(result.content).replace("|", "\\|").replace("\n", " ")[:200]
                    + "..."
                )
                summary = (
                    str(result.summary).replace("|", "\\|") if result.summary else "N/A"
                )
                metadata = (
                    str(result.metadata).replace("|", "\\|")
                    if result.metadata
        ...
        if openai_analysis:
            markdown_content.extend(
                ["### OpenAI Analysis", "```", openai_analysis, "```\n"]
            )

        if groq_analysis:
            markdown_content.extend(
                ["### Groq Analysis", "```", groq_analysis, "```\n"]
            )
...
def search_all(query, max_results=30, skip_prompts=False, run_analysis=True):
    ...
    try:
        ...
        fine_grained_results = (
            client.rpc(
                "dot_product_search",
                {
                    ...
                    "match_count": int(
                        max_results * fine_grained_params["result_percentage"]
                    ),
                    "content_weight": fine_grained_params["content_weight"],
                    "summary_weight": fine_grained_params.get(
                        "summary_weight", 1.0 - fine_grained_params["content_weight"]
                    ),
                },
            )
            .execute()
        ...
        contextual_results = (
            client.rpc(
                "dot_product_search",
                {
                    "query_embedding": embedding,
                    "match_count": int(
                        max_results * contextual_params["result_percentage"]
                    ),
                    "content_weight": contextual_params["content_weight"],
                    "summary_weight": contextual_params.get(
                        "summary_weight", 1.0 - contextual_params["content_weight"]
                    ),
                },
            )
            .execute()
        ...
        overview_results = (
            client.rpc(
                "dot_product_search",
                {
                    "query_embedding": embedding,
                    "match_count": int(
                        max_results * overview_params["result_percentage"]
                    ),
                    "content_weight": overview_params["content_weight"],
                    "summary_weight": overview_params.get(
                        "summary_weight", 1.0 - overview_params["content_weight"]
                    ),
                },
            )
            .execute()
        ...
        console.print(
            f"[green]Fine-grained segments: {len(fine_grained_results)} results in {fine_grained_time:.2f}s[/green]"
        )
        console.print(
            f"[green]Contextual segments: {len(contextual_results)} results in {contextual_time:.2f}s[/green]"
        )
        console.print(
            f"[green]Overview segments: {len(overview_results)} results in {overview_time:.2f}s[/green]"
        )
    ...
    try:
        ...
        keyword_results_raw = (
            client.rpc(
                "keyword_search", {"query_text": query, "match_count": max_results}
            )
            .execute()
            .data
        )
        ...
        console.print(
            f"[green]Keyword search: {len(keyword_results_raw)} results in {keyword_time:.2f}s[/green]"
        )
    ...
    try:
        ...
        if dot_product_results and keyword_results:
            ...
            console.print(
                f"[green]Hybrid search: {len(hybrid_results)} results in {hybrid_time:.2f}s[/green]"
            )
    ...
    if all_results:
        ...
        for i, result in enumerate(all_results[:5], 1):
            ...
            method_style = SEARCH_METHOD_STYLES.get(
                search_method, SEARCH_METHOD_STYLES["default"]
            )
            ...
            score_color = (
                "green" if similarity > 0.8 else "yellow" if similarity > 0.6 else "red"
            )

            # Print result with better formatting
            console.print(
                f"[bold]{i}.[/bold] [bold {score_color}]{similarity:.3f}[/bold {score_color}] "
    ...
    if run_analysis and all_results:
        ...
        try:
            ...
            console.print(
                f"[blue]Generation Input Tokens:[/] {token_usage['generation_tokens']['input']:,}"
            )
            console.print(
                f"[blue]Generation Output Tokens:[/] {token_usage['generation_tokens']['output']:,}"
            )
        except Exception as e:
        ...
        # Groq Analysis
        try:
            groq_analysis = analyze_search_results(
                all_results, provider="groq", max_results=10
            )
        except Exception as e:
    ...
    if not skip_prompts:
        ...
        if save_option != "n":
            ...
            if format_choice in format_options:
                save_results(
                    all_results, query, format_choice, openai_analysis, groq_analysis
                )
            else:
                console.print(
                    f"[yellow]Invalid format choice. Using default (md).[/yellow]"
                )
...
def display_results_table(results: List[SearchResult], title: str):
    ...
    table.add_column(
        "Score",
        justify=TABLE_STYLES["columns"]["score"]["justify"],
        style=TABLE_STYLES["columns"]["score"]["style"],
        width=TABLE_STYLES["columns"]["score"]["width"],
    )
    table.add_column(
        "Method",
        style=TABLE_STYLES["columns"]["method"]["style"],
        width=TABLE_STYLES["columns"]["method"]["width"],
    )
    table.add_column(
        "Source",
        style=TABLE_STYLES["columns"]["source"]["style"],
        width=TABLE_STYLES["columns"]["source"]["width"],
    )
    table.add_column(
        "Video ID",
        style=TABLE_STYLES["columns"]["video_id"]["style"],
        width=TABLE_STYLES["columns"]["video_id"]["width"],
    )
    table.add_column(
        "Content",
        style=TABLE_STYLES["columns"]["content"]["style"],
        width=TABLE_STYLES["columns"]["content"]["width"],
    )
    table.add_column(
        "Summary",
        style=TABLE_STYLES["columns"]["metadata"]["style"],
        width=TABLE_STYLES["columns"]["metadata"]["width"],
    )
    table.add_column(
        "Segment ID",
        style=TABLE_STYLES["columns"]["segment_id"]["style"],
    ...
    table.add_column(
        "Start Time",
        style=TABLE_STYLES["columns"]["start_time"]["style"],
        width=TABLE_STYLES["columns"]["start_time"]["width"],
    )
    table.add_column(
        "End Time",
        style=TABLE_STYLES["columns"]["end_time"]["style"],
        width=TABLE_STYLES["columns"]["end_time"]["width"],
    )
    table.add_column(
        "Watch URL",
        style=TABLE_STYLES["columns"]["watch_url"]["style"],
    ...
    console.print(table)
    console.print(
        f"{STATUS_INDICATORS['success']} [green]Found {len(results)} results[/green]"
    )
    console.print("\n")
...
def display_tier_results(results: List[SearchResult], tier_name: str):
    ...
    for result in results:
        ...
        table.add_row(f"{result.similarity:.3f}", content, result.source, timestamp)
...
def display_results(results: List[SearchResult], show_by_type: bool = True):
    ...
    if len(results) > max_results_for_analysis:
        console.print(
            f"[yellow]Warning: Limiting analysis to the top {max_results_for_analysis} most relevant results (out of {len(results)}) to prevent token overflow.[/yellow]"
        )
        analysis_results = sorted(
            results,
            key=lambda r: r.similarity if r.similarity is not None else 0,
            reverse=True,
        )[:max_results_for_analysis]
    else:
    ...
    try:
        ...
        if "Error" in openai_analysis and (
            "token" in openai_analysis or "context length" in openai_analysis
        ):
            console.print(
                Panel(
                    str(openai_analysis),
            ...
            if len(analysis_results) > 10:
                console.print(
                    "[yellow]Retrying OpenAI analysis with fewer results...[/yellow]"
                )
                ...
                console.print(
                    Panel(
                        str(retry_analysis),
                        title="OpenAI Analysis (Retry with 10 results)",
                        border_style="green",
                        highlight=True,
                    )
                )
    ...
    try:
        ...
        if "Error" in groq_analysis and (
            "token" in groq_analysis or "too large" in groq_analysis
        ):
            console.print(
                Panel(str(groq_analysis), title="Groq Analysis", border_style="yellow")
            )

            # If we encounter token limits, try with even fewer results
            if len(analysis_results) > 5:
                console.print(
                    "[yellow]Retrying Groq analysis with fewer results...[/yellow]"
                )
                retry_results = analysis_results[
                    :5
                ]  # Groq has stricter limits, so use fewer results
                retry_analysis = analyze_search_results(retry_results, "groq")
                console.print(
                    Panel(
                        str(retry_analysis),
                        title="Groq Analysis (Retry with 5 results)",
                        border_style="blue",
                    )
                )
        else:
            console.print(
                Panel(str(groq_analysis), title="Groq Analysis", border_style="blue")
            )
    except Exception as e:
        console.print(f"\n[red]Groq Analysis Error: {str(e)}[/red]")
    ...
    if show_by_type:
        ...
        for source in source_order:
            if source in results_by_source:
                ...
                console.print(
                    f"[green]Found {len(source_results)} results from {source}[/green]"
                )
        ...
        [
            {
                ...
                if result.source == "document_embeddings"
                else None,
...
def format_row_data(row: pd.Series) -> Dict[str, str]:
    ...
    if row["Source"] == "video_transcriptions":
        ...
        line_num = (
            row["Metadata"].get("line_number", "") if pd.notna(row["Metadata"]) else ""
        )
        ...
        url_info = (
            ...
            if pd.notna(row["Metadata"]) and "url" in row["Metadata"]
            else ""
        )
        ...
        content = str(row["Content"])
        title_info = (
            f"[{row['Metadata'].get('title', '')}] "
            if pd.notna(row["Metadata"]) and "title" in row["Metadata"]
            else ""
        )
        ...
        content = (
            str(row["Content"])[:max_width] + "..."
            if len(str(row["Content"])) > max_width
    ...
    return {
        ...
        "method": f"{SEARCH_METHOD_STYLES.get(row['Method'], SEARCH_METHOD_STYLES['default'])['icon']} {row['Method']}"
        if row["Method"]
        else "unknown",
        "source": f"{SOURCE_STYLES.get(row['Source'], SOURCE_STYLES['default'])['icon']} {row['Source']}"
        if row["Source"]
        else "unknown",
        "video_id": str(row["Video ID"]),
        "content": content,
        "summary": str(row["Summary"])
        if row["Source"] == "document_embeddings" and pd.notna(row["Summary"])
        else "N/A",
    ...
    while True:
        ...
        if choice == "1":
            ...
            format_choice = Prompt.ask(
                "Export format", choices=["csv", "excel"], default="csv"
            )
            filename = Prompt.ask(
                "Filename",
                default=f"search_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            )
...
def main():
    ...
    while True:
        ...
        console.print(
            f"3. [bold]Analysis[/bold] - Toggle analysis {'[green]ON[/green]' if run_analysis else '[red]OFF[/red]'}"
        )
        ...
        console.print(
            f"🔍 Total max results: {sum(params[tier]['max_results'] for tier in params)}"
        )
        console.print(
            f"🧠 AI Analysis: {'[green]Enabled[/green]' if run_analysis else '[red]Disabled[/red]'}"
        )
        ...
        if choice == "1":
            ...
            results, openai_analysis, groq_analysis = search_all(
                query,
            ...
            if run_analysis:
                ...
                console.print(
                    f"[blue]Embedding Tokens:[/blue] {token_usage['embedding_tokens']:,}"
                )
                console.print(
                    f"[blue]Generation Input Tokens:[/blue] {token_usage['generation_tokens']['input']:,}"
                )
                console.print(
                    f"[blue]Generation Output Tokens:[/blue] {token_usage['generation_tokens']['output']:,}"
                )
                console.print(
                    f"[blue]Total Tokens:[/blue] {token_usage['embedding_tokens'] + token_usage['generation_tokens']['input'] + token_usage['generation_tokens']['output']:,}"
                )
            ...
            if run_analysis:
                console.print(
                    "Search results will be analyzed using AI models (OpenAI & Groq)."
                )
                console.print(
                    "You'll see a token usage preview before analysis starts."
                )
            else:
        ...
        if param_name.endswith("threshold") or param_name.endswith("weight"):
            ...
            try:
                ...
                if 0.0 <= new_value <= 1.0:
                    ...
                    console.print(
                        "[yellow]Value must be between 0.0 and 1.0. No changes made.[/yellow]"
                    )
            except ValueError:
            ...
            console.print(
                "[yellow]Note: Values should be between 0.0 and 1.0 (represents 0-100%)[/yellow]"
            )
            try:
                new_value = float(input().strip() or str(current_value))
                if 0.0 <= new_value <= 1.0:
            ...
            try:
                ...
                if 1 <= new_value <= 100:
                    ...
                    console.print(
                        "[yellow]Value must be between 1 and 100. No changes made.[/yellow]"
                    )
            except ValueError:
                console.print("[yellow]Invalid value. No changes made.[/yellow]")
...
def load_preset_menu():
    ...
    choice = input(
        "\nSelect preset to load (number or name, blank to cancel): "
    ).strip()
...
def save_preset_menu():
    ...
    if preset_name in global_search_params.list_presets():
        confirm = (
            input(f"Preset '{preset_name}' already exists. Overwrite? (y/n): ")
            .strip()
            .lower()
        )
        if confirm != "y":
