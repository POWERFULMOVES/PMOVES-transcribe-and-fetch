...
class SearchResult:
    ...
    @classmethod
    def from_db_result(cls, result: Dict[str, Any], source: str) -> 'SearchResult':
        """Create a SearchResult from a database result."""
        # Ensure we have a source
        result['source'] = source

        # Handle content field based on source
        if source == 'video_transcriptions_full':
            result['content'] = result.get('full_transcript', '')
        elif 'text' in result and not result.get('content'):
        ...
        if 'id' in result:
            ...
            result['id'] = str(result.get('video_id', '')) + '_' + str(result.get('segment_id', ''))
