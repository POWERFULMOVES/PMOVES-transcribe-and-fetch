...
async def process_video(youtube_video_url, obsidian_dir, status_updates, transcription_updates,
    ...
    try:
        ...
        if use_groq:
            ...
            logger.info(f"Sending request to Groq API with audio file: {audio_output_path}")
            logger.info(f"Processing audio file with Groq API: {audio_output_path}")
            
            try:
                # Use audio_output_path consistently
                transcription = await process_audio_in_chunks(audio_output_path)
...
async def transcribe_audio(audio_path: str, status_updates: asyncio.Queue, transcription_updates: asyncio.Queue, youtube_video_url: str):
    try:
        ...
        segments = list(segments_gen)  # Convert generator to list immediately
        ...
        for idx, segment in enumerate(segments):
            ...
            # Send progress update every few segments
            if idx % 5 == 0:
                await status_updates.put(json.dumps({
                    "type": "status",
                    "content": f"Transcribing segment {idx + 1} of {len(segments)}"
                }))
