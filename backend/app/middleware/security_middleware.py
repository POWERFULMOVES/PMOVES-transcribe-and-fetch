...
if not VALID_API_KEYS:
    ...
    logger.info(f"API Key Authentication enabled. {len(VALID_API_KEYS)} valid key(s) loaded.")
...
SERVER_API_KEY = os.getenv("BACKEND_API_KEY") # Used by backend to protect its own endpoints
...
if not ALLOWED_API_KEYS:
    logger.warning("No API keys configured in ALLOWED_API_KEYS or BACKEND_API_KEY. API key security will be ineffective.")
...
class APIKeySecurityMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        if request.method == "OPTIONS":
            logger.debug(f"Handling OPTIONS request for {request.url.path}, bypassing API key check.")
            ...
            return Response(status_code=status.HTTP_200_OK, headers={
                "Access-Control-Allow-Origin": request.headers.get("Origin", "*"), # Be more specific if possible
                "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS, PATCH", # Mirror main.py
                "Access-Control-Allow-Headers": request.headers.get("Access-Control-Request-Headers", "*"), # Reflect requested headers
        ...
        if auth_header:
            ...
            if len(parts) == 2 and parts[0].lower() == "bearer":
                ...
                logger.debug(f"Extracted API key from Authorization Bearer header for {request.url.path}")
        ...
        if not api_key_received:
            ...
            if custom_api_key:
                ...
                logger.debug(f"Extracted API key from {API_KEY_NAME} header for {request.url.path}")

        if not api_key_received:
            logger.warning(f"API key missing from both Authorization and {API_KEY_NAME} headers for {request.url.path}. Denying access.")
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
                content={"detail": "API key required. Provide via 'Authorization: Bearer <key>' or 'X-API-KEY: <key>' header."},
            )

        if api_key_received not in ALLOWED_API_KEYS:
            logger.warning(f"Invalid API key provided for {request.url.path}. Key: '{api_key_received[:5]}...'. Denying access.")
            return JSONResponse(
