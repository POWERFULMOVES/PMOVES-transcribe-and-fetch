...
from ...models.tool_calling_models import ToolCallState, ToolCallStatus
import logging
...
class ToolCallStateStore:
    ...
    async def stop_cleanup_worker(self):
        if self._cleanup_task and not self._cleanup_task.done():
            ...
            self.logger.info(
                "ToolCallStateStore cleanup worker not running or already stopped."
            )
    ...
    async def create_state(self, initial_state: ToolCallState) -> None:
        ...
        async with lock:
            if initial_state.tool_call_id in self._active_states:
                raise ValueError(
                    f"Tool call state with ID {initial_state.tool_call_id} already exists."
                )
            ...
            initial_state.last_activity_at = (
                current_time  # Always set/update last_activity_at on creation
            )
    ...
    async def get_state(self, tool_call_id: str) -> Optional[ToolCallState]:
        ...
        async with lock:
            ...
            if state:
                ...
                self.logger.debug(
                    f"Tool call state retrieved, last_activity_at updated: {tool_call_id}"
                )
            return state

    async def update_state(
        self, tool_call_id: str, updates: Dict[str, Any]
        ...
        async with lock:
            ...
            if not existing_state:
                self.logger.warning(
                    f"Attempted to update non-existent state: {tool_call_id}"
                )
                return None
            ...
            try:
                ...
                self.logger.debug(
                    f"Tool call state updated: {tool_call_id}, updates: {updates}"
                )
                return updated_state
            except (
                Exception
            ) as e:  # Broad exception for Pydantic validation or other errors
                self.logger.error(
                    f"Error updating state for {tool_call_id}: {e}. Updates: {updates_with_activity}"
                )
                return None
    ...
    async def delete_state(self, tool_call_id: str) -> bool:
        ...
        async with lock:
            if tool_call_id in self._active_states:
                ...
                return True
            self.logger.warning(
                f"Attempted to delete non-existent state: {tool_call_id}"
            )
            return False
    ...
    async def _periodic_cleanup(self):
        ...
        try:
            while True:
                ...
                for tool_call_id in tool_call_ids_to_check:
                    lock = await self._get_lock(
                        tool_call_id
                    )  # Get lock for this specific state
                    async with lock:
                        state = self._active_states.get(tool_call_id)
                        if (
                            not state
                        ):  # State might have been deleted since we got the keys
                            continue
                        ...
                        if (
                            ...
                            self.logger.info(
                                f"Tool call {tool_call_id} exceeded total timeout ({self.total_timeout_seconds}s)."
                            )

                        # Check for idle timeout
                        elif (
                            now - state.last_activity_at
                        ).total_seconds() > self.idle_timeout_seconds:
                            timed_out = True
                            reason = "idle activity"
                            self.logger.info(
                                f"Tool call {tool_call_id} exceeded idle timeout ({self.idle_timeout_seconds}s)."
                            )

                        if timed_out:
                            self.logger.info(
                                f"Timing out state {tool_call_id} due to {reason}."
                            )
                            state.status = ToolCallStatus.TIMEOUT
                            state.last_activity_at = (
                                now  # Update activity time for the timeout event
                            )
                            # No need to call self.update_state as we have the lock and will delete it

                            # Delete the state
                            if (
                                tool_call_id in self._active_states
                            ):  # Re-check before delete
                                del self._active_states[tool_call_id]
                            if tool_call_id in self._locks:  # And its lock
                                del self._locks[tool_call_id]
                            self.logger.info(
                                f"Tool call state {tool_call_id} deleted due to {reason} timeout."
                            )
            ...
            self.logger.error(
                f"Unexpected error in periodic cleanup: {e}", exc_info=True
            )
