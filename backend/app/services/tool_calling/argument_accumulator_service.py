...
from ...models.tool_calling_models import (
    ToolCallState,
    ToolCallStatus,
...
class ArgumentAccumulatorService:
    def __init__(
        self,
        state_store: ToolCallStateStore,
        ...
        self._queue_locks: Dict[
            str, asyncio.Lock
        ] = {}  # For managing access to _notification_queues entries
    ...
    async def _get_or_create_notification_queue(
        ...
        async with lock:
            if tool_call_id not in self._notification_queues:
                ...
                self.logger.debug(
                    f"Notification queue created for tool_call_id: {tool_call_id}"
                )
            return self._notification_queues[tool_call_id]

    async def subscribe_to_notifications(self, tool_call_id: str) -> asyncio.Queue:
        self.logger.info(
            f"New subscription for notifications for tool_call_id: {tool_call_id}"
        )
        ...
        async with lock:
            if tool_call_id in self._notification_queues:
                try:
                    ...
                    self.logger.debug(
                        f"Notified subscribers for {tool_call_id} with event: {event.get('status')}"
                    )
                    ...
                    if status in [
                        ...
                        if (
                            tool_call_id in self._queue_locks
                        ):  # Should always be true if queue existed
                            del self._queue_locks[tool_call_id]
                        self.logger.info(
                            f"Notification queue and lock cleaned up for terminal event {status} for {tool_call_id}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Error notifying subscribers for {tool_call_id}: {e}",
                        exc_info=True,
                    )
            else:
                self.logger.debug(
                    f"No active notification queue for {tool_call_id} to send event: {event.get('status')}"
                )
    ...
    async def initiate_tool_call(self, request: InitiateToolCallRequest) -> str:
        ...
        try:
            ...
            self.logger.info(
                f"Tool call initiated: {tool_call_id} for tool {request.tool_name}"
            )
            return tool_call_id
            ...
            self.logger.error(
                f"Unexpected error initiating tool call {tool_call_id}: {e}",
                exc_info=True,
            )
    ...
    async def submit_argument_chunk(
        self, request: SubmitArgumentChunkRequest
    ) -> Tuple[bool, Optional[str]]:
        state = await self.state_store.get_state(request.tool_call_id)

        if not state:
            self.logger.warning(
                f"Submit chunk: Tool call ID {request.tool_call_id} not found."
            )
        ...
        if request.is_last_chunk:
            self.logger.info(
                f"Last chunk received for {request.tool_call_id}. Attempting reassembly."
            )
            state.status = ToolCallStatus.REASSEMBLING  # Mark reassembling
            await self.state_store.update_state(
                request.tool_call_id, {"status": state.status}
            )
            ...
            if (
                state.expected_chunk_count is not None
                and len(sorted_sequence_numbers) != state.expected_chunk_count
            ):
                error_msg = f"Missing chunks: expected {state.expected_chunk_count}, got {len(sorted_sequence_numbers)}"
                self.logger.error(
                    f"Reassembly failed for {request.tool_call_id}: {error_msg}"
                )
                state.status = ToolCallStatus.FAILED
                state.error_details = error_msg
                await self.state_store.update_state(
                    request.tool_call_id,
                    {"status": state.status, "error_details": state.error_details},
                )
            ...
            full_json_string = "".join(
                state.partial_arg_buffers[i] for i in sorted_sequence_numbers
            )
            ...
            try:
                ...
                self.logger.debug(
                    f"JSON decoded successfully for {request.tool_call_id}"
                )
            ...
            is_valid, validation_error_msg = (
                self.validation_service.validate_accumulated_arguments(
                    state.tool_schema, parsed_args
                )
            )
            if not is_valid:
                self.logger.error(
                    f"Validation failed for {request.tool_call_id}: {validation_error_msg}"
                )
                state.status = ToolCallStatus.FAILED
            ...
            await self.state_store.update_state(
                request.tool_call_id,
                {
                    ...
                    "status": state.status,
                    "partial_arg_buffers": state.partial_arg_buffers,
                },
            )
            await self._notify_subscribers(
                request.tool_call_id,
                {
                    "status": ToolCallStatus.ARGUMENTS_COMPLETE,
                    "data": parsed_args,
                    "tool_call_id": request.tool_call_id,
                },
            )
            ...
            self.logger.debug(
                f"Chunk {request.sequence_number} accumulated for {request.tool_call_id}"
            )
    ...
    async def cancel_tool_call(self, tool_call_id: str) -> bool:
        ...
        if not state or state.status in [
            ...
            self.logger.warning(
                f"Cancel request for {tool_call_id}: Not found or already in terminal state ({state.status if state else 'N/A'})."
            )
            return False
        ...
        await self._notify_subscribers(
            tool_call_id,
            {"status": ToolCallStatus.CANCELLED, "tool_call_id": tool_call_id},
        )
    ...
    async def process_timeout(self, tool_call_id: str):
        ...
        state = await self.state_store.get_state(
            tool_call_id
        )  # State store updates last_activity_at

        if state and state.status == ToolCallStatus.TIMEOUT:
            self.logger.warning(
                f"Processing timeout notification for tool call {tool_call_id}."
            )
            await self._notify_subscribers(
                tool_call_id,
                {
                    "status": ToolCallStatus.TIMEOUT,
                    "error": state.error_details or "Tool call timed out",
                    "tool_call_id": tool_call_id,
                },
            )
        elif state:
            self.logger.info(
                f"process_timeout called for {tool_call_id}, but status is {state.status}, not TIMEOUT."
            )
        else:
            self.logger.warning(
                f"process_timeout called for non-existent tool call ID: {tool_call_id}"
            )

    async def get_tool_call_state_details(
        self, tool_call_id: str
