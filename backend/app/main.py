...
try:
    ...
    from .ollama_initializer import ensure_ollama_model_loaded # Import the Ollama initializer
...
class FetchHistoryBase(BaseModel):
    ...
    output_type: Optional[str] = Field(None, description="The type of output generated (e.g., 'markdown', 'pdf_link', 'json', 'unknown').") # ADDED
...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {"name": "LLM Endpoints", "description": "Endpoints for interacting with LLM capabilities."},
    ]
)
    ...
    try:
        ...
        if PROJECT_MODULES_LOADED and 'ensure_ollama_model_loaded' in globals():
            try:
                ...
                logger.error(f"Error during Ollama model initialization: {e_ollama_init}", exc_info=True)
        else:
            logger.warning("Ollama initializer function (ensure_ollama_model_loaded) not found in globals or PROJECT_MODULES_LOADED is false. Ollama model loading on startup will be skipped.")
...
async def get_embedding_with_registry(text: str, model_id: str = os.getenv("DEFAULT_EMBEDDING_MODEL_ID", "openai/text-embedding-ada-002")) -> Optional[List[float]]:
    ...
    try:
        ...
        logger.error(f"LLM Registry service not initialized: {e}. Cannot generate embedding.")
    ...
    try:
        embedding = await registry_service.generate_embedding(model_id=model_id, text=text) # Call method on instance
        if embedding:
...
async def fetch_content_endpoint(
    ...
    async def event_generator():
        ...
        try:
            ...
            if engine.lower() == "crawl4ai":
                ...
                async for sse_event_json_str_from_fetcher in fetch_with_crawl4ai(url=url, original_request_params=all_request_params):
                    try:
                        ...
                        logger.info(f"About to yield for crawl4ai: {json.dumps(final_sse_event_dict)}")
            ...
            if fetched_data_dict and not fetched_data_dict.get("error"): # If no error at the top level of fetched_data_dict
                ...
                llm_extracted_content = fetched_data_dict.get('extracted_content') # Get potential LLM content
                final_content_for_sse = llm_extracted_content if llm_extracted_content is not None else markdown_content

                if not final_content_for_sse: 
                    logger.warning(f"Engine {engine} returned no primary content (markdown or extracted) for URL: {url}")
                else:
                    logger.info(f"Successfully fetched primary content using {engine} from {url}. Title: '{title}'. Length: {len(final_content_for_sse)}")
                ...
                if generate_pdf:
                    ...
                    if final_content_for_sse and pdf_path_from_fetcher: 
                        ...
                        try:
                            ...
                            output_pdf_filepath = PDF_STORAGE_BASE_DIR / unique_pdf_filename 
                            
                            logger.info(f"Attempting PDF generation for {fetched_url_actual} to {output_pdf_filepath}")
                            pdf_conversion_successful = await generate_pdf_from_markdown_string(final_content_for_sse, str(output_pdf_filepath))
                ...
                if upload_to_supabase:
                    ...
                    if final_content_for_sse: 
                        ...
                        embedding = await get_embedding_with_registry(final_content_for_sse, model_id=default_embedding_model_id)
                        if embedding:
                            ...
                            supabase_content_id = await upsert_content_to_supabase(
                                url=fetched_url_actual, title=title, markdown_content=final_content_for_sse, 
                ...
                if fetch_history_id and supabase_client_for_history:
                    ...
                    final_processed_content_path = content_storage_path_local # Default to markdown/text path

                    if pdf_relative_path: # If a PDF was generated
                        current_output_type = "pdf"
                        final_processed_content_path = pdf_relative_path
                    elif output_format: # output_format is a Query param for /fetch-content
                        current_output_type = output_format.lower()
                        # final_processed_content_path remains content_storage_path_local (for md, txt etc)
                    elif markdown_content: # Fallback if no explicit format but markdown exists
                        current_output_type = "markdown"
                        # final_processed_content_path remains content_storage_path_local
                    
                    success_update_data = {
                        "status": "success",
                        "content_summary": (final_content_for_sse or "")[:500] + "..." if final_content_for_sse and len(final_content_for_sse) > 500 else (final_content_for_sse or "No content."),
                        "raw_content_path": raw_content_path_local if raw_content_path_local and raw_content_path_local != final_processed_content_path else None,
                ...
                final_payload_data = {
                    ...
                    "markdown_content_fallback": markdown_content if final_content_for_sse != markdown_content and llm_extracted_content is not None else None,
                    "content_preview": (final_content_for_sse or "")[:250] + "..." if final_content_for_sse else "No content.",
            ...
            if fetch_history_id and supabase_client_for_history and not terminal_status_written_to_db:
                ...
                _, db_write_ok = await _update_fetch_history_record(supabase_client_for_history, fetch_history_id, failure_update_data, all_request_params)
                if db_write_ok and failure_update_data.get("status") in ["success", "failed"]: # Check status is terminal
...
if PROJECT_MODULES_LOADED:
    ...
    if 'llm_router' in locals() and llm_router: # Added for new LLM endpoints
        try: app.include_router(llm_router, prefix="/api/v1", tags=["LLM Endpoints"])
        except Exception as e: logger.warning(f"Failed to include llm_router: {e}")
