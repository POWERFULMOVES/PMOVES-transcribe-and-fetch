...
# If this module is imported elsewhere, basicConfig might be called multiple times.
# For robustness, use getLogger and let the application configure handlers.
...
try:
    ...
    logger.warning("LLMRegistryService components could not be imported. LLM functionalities will be disabled.")
...
class AgentFramework:
    def __init__(
        ...
        if llm_service_available and self.llm_registry_url:
            # The LLMRegistryService is a singleton managed by its own module's functions.
            # AgentFramework doesn't instantiate it directly but will use its getter after initialization.
            logger.info(f"AgentFramework configured to use LLMRegistryService with proxy URL: {self.llm_registry_url}")
            # Initialization of the service and its cache refresh will be handled in self.start_services()
    ...
    async def start_services(self):
        ...
        if llm_service_available and self.llm_registry_url:
            try:
                # Initialize the singleton LLMRegistryService, potentially with an override URL
                await initialize_llm_registry(litellm_proxy_url_override=self.llm_registry_url)
                self._llm_service_instance = get_llm_registry_service() # Get the initialized instance
        ...
        elif llm_service_available and not self.llm_registry_url: # Use default config
             try:
